...
public class NPLoader implements Loader{
	private static final Logger LOG = LoggerFactory.getLogger(NPLoader.class) ;
	...
	public ResponseBean load(String path,String loadCurrency) throws IOException{
		...
		try{
	        ...
	        for(int it = pageSize-1 ; it >= 0 ; it--){
	        	 ...
	        	 if(text.contains("COMBINED") && (text.replace(" ", "").contains("NEPAL"+loadCurrency) || text.replace(" ", "").contains("PHILIPPINES"+loadCurrency))){
	        		 targetText = text ; 
	        		 if(0 <= it+1 && it+1 < pageSize && !targetText.replace(" ", "").contains("ENDOFCURRENCY")){
	        			 targetText += stripper.getText(pages.get(it+1)) ;
	        		 }
	        		 break ; 
	        	 } 
	        }
